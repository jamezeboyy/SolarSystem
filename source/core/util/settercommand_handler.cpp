...
namespace SolarSystem
{
    namespace Core
    {
        ...
        {
            this->mHandlerMap = {
                    {"OBJ1", std::bind(&CSetterCommandHandler::obj_set_handler, this, std::placeholders::_1)}
            };
        }

        std::string CSetterCommandHandler::callback_handler(std::vector<std::string> parameters)
        {
        ...
        // Example
        std::string CSetterCommandHandler::obj_set_handler(const std::vector<std::string>& parameters)
        {
